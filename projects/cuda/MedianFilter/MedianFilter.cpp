...
//
// Cuda example code that implements the Median filter
...
// Other keys:
// I: display image
// T: display Median filter (computed solely with texture)
...
const char *sSDKsample = "CUDA Median filter";
...
enum MedianDisplayMode g_MedianDisplayMode;
...
void computeFPS()
{
    ...
    if (fpsCount == fpsLimit)
    {
        ...
        sprintf(fps, "CUDA Median filter (%s): %3.1f fps", filterMode[g_MedianDisplayMode], ifps);
...
void display(void)
{
    ...
    medianFilter(data, imWidth, imHeight, g_MedianDisplayMode, imageScale);
...
void keyboard(unsigned char key, int /*x*/, int /*y*/)
{
    ...
    switch (key)
    {
        ...
        case 'i':
        case 'I':
            g_MedianDisplayMode = MEDIANDISPLAY_IMAGE;
            sprintf(temp, "CUDA Median filter (%s)", filterMode[g_MedianDisplayMode]);
            glutSetWindowTitle(temp);
            break;

        case 't':
        case 'T':
            g_MedianDisplayMode = MEDIANDISPLAY_SOBELTEX;
            sprintf(temp, "CUDA Median filter (%s)", filterMode[g_MedianDisplayMode]);
            glutSetWindowTitle(temp);
            break;
...
void loadDefaultImage(char *loc_exec)
{
    ...
    printf("Reading image: untitled.pgm\n");
    const char *image_filename = "untitled.pgm";
...
void initGL(int *argc, char **argv)
{
    ...
    glutCreateWindow("CUDA Median filter");
...
void runAutoTest(int argc, char *argv[])
{
    ...
    switch (mode)
    {
        ...
        case 2:
            g_MedianDisplayMode = MEDIANDISPLAY_SOBELSHARED;
    ...
    printf("AutoTest: %s <%s>\n", sSDKsample, filterMode[g_MedianDisplayMode]);
    medianFilter(d_result, imWidth, imHeight, g_MedianDisplayMode, imageScale);
...
int main(int argc, char **argv)
{
    ...
    printf("T: display Median filter (Using Texture)\n");
